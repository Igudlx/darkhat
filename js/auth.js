...
async function handleSubmit(e) {
  ...
  try {
    ...
    if (mode === "login") {
      ...
      // The profile-doc write and the "Logging In..." animation don't depend
      // on each other — run them at the same time instead of back-to-back.
      await Promise.all([
        ensureUserDoc(cred.user.uid, email).catch((docErr) => {
...
async function initAuth() {
  ...
  const { auth, AuthLib } = fb();
  // "Local" persistence is already the web default, but set it explicitly —
  // fire-and-forget, since registering the state listener below shouldn't
  // have to wait on it.
  AuthLib.setPersistence(auth, AuthLib.browserLocalPersistence).catch((err) => {
    console.warn("Couldn't set auth persistence (falling back to default):", err);
  });
  ...
  AuthLib.onAuthStateChanged(auth, async (user) => {
    ...
    bootHandled = true;
    if (user) {
      // Same idea as signup: don't make a returning user wait through the
      // profile check and then the animation — run them together.
      await Promise.all([
        ensureUserDoc(user.uid, user.email).catch((docErr) => {
