...
async function applySettings() {
  let doc = {};
  try {
    doc = await FS.getUserDoc();
  } catch (err) {
    console.warn("Couldn't load settings from Firestore, using defaults:", err);
  }
...
async function build() {
  ...
  presetSelect.addEventListener("change", async () => {
    try {
      ...
      console.warn("Couldn't save background preset (check Firestore rules are deployed):", err);
    }
    applySettings();
  });
  ...
  blurSlider.addEventListener("change", async () => {
    try {
      ...
      console.warn("Couldn't save background blur (check Firestore rules are deployed):", err);
    }
  });
