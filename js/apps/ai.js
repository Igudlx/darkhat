...
async function renderSidebarList() {
  let chats = [];
  try {
    chats = await FS.listChats();
  } catch (err) {
    console.warn("Couldn't load chats (check Firestore rules are deployed):", err);
  }
...
async function open() {
  ...
  try {
    ...
    console.warn("Couldn't load saved API key (check Firestore rules are deployed):", err);
  }
  renderSidebarList();
}
