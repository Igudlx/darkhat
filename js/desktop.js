...
export async function renderDesktop() {
  ...
  // Default apps must never wait on a network round trip — render them the
  // instant this runs, in their default auto-placed positions.
  const defaultEls = {};
  DEFAULT_APPS.forEach((app) => {
    const el = buildIconEl(app);
    wireIconEvents(el, app.id, app.open, { deletable: false });
    container.appendChild(el);
    defaultEls[app.id] = el;
  });

  // Saved icon positions and any files pinned to the desktop load in the
  // background, in parallel (not one-after-the-other), and get layered in
  // as soon as they're ready without blocking the icons above.
  const [layoutResult, filesResult] = await Promise.allSettled([
    FS.getUserDoc(),
    FS.listFiles("desktop"),
  ]);

  if (layoutResult.status === "fulfilled") {
    layoutCache = layoutResult.value.desktopLayout || {};
    Object.entries(defaultEls).forEach(([id, el]) => placeIcon(el, layoutCache[id]));
  } else {
    layoutCache = {};
    console.warn("Couldn't load saved desktop layout (check that firestore.rules is deployed):", layoutResult.reason);
  }

  const files = filesResult.status === "fulfilled" ? filesResult.value : [];
  if (filesResult.status === "rejected") {
    console.warn("Couldn't load desktop files (check that firestore.rules is deployed):", filesResult.reason);
  }
